...
OptionSettings
SettingsWithOverrides(const char *optionId,
                      std::map<std::string, std::string> values) {
  const ModifierRegistration *const registration = FindModifier(optionId);
...
void TestCatalogAtoms() {
  const auto checkAtom = [](const std::string &id, BlockShape shape,
  ...
  checkAtom("evaluate/speed-toward", BlockShape::Reporter, "evaluation",
            kVelocityEvaluationId);
  checkAtom("evaluate/distance-to-point", BlockShape::Reporter, "evaluation",
            kPointTargetEvaluationId);
  checkAtom("evaluate/distance-to-pose", BlockShape::Reporter, "evaluation",
            kPoseTargetEvaluationId);
  checkAtom("evaluate/box-entry-time", BlockShape::Reporter, "evaluation",
            kVolumeEntryEvaluationId);
  checkAtom("evaluate/prism-entry-time", BlockShape::Reporter, "evaluation",
  ...
  for (const BlockDefinition &definition : BlockCatalog()) {
    ...
    if (definition.optionKind == "mutation") {
      ...
      if (registration != nullptr)
        defaults = &registration->defaultSettings;
      ...
      Check(registration != nullptr, "atom binds registered evaluator");
      if (registration != nullptr)
        defaults = &registration->defaultSettings;
...
void TestProgramOperations() {
  ...
  Check(program.find(group)->substack.front() == second, "moved atom first");

  Check(program.removeBlock(second), "remove atom");
  Check(program.find(group)->substack.size() == 1, "one atom after remove");
  ...
  const BlockId reporter = program.attachReporter(
      group, "seed", "values/add", {{"left", "1179926867"}, {"right", "1"}});
...
void TestLoweringEquivalence() {
  ...
  const auto check = [&detail](const SearchComponentConfiguration &components,
                               const char *what) {
    if (RoundTripsComponents(components, &detail))
      return;
  ...
  const OptionConfiguration evaluation = DefaultEvaluationTargetConfiguration();

  // Every modifier's default configuration round-trips.
  for (const ModifierRegistration &registration : ModifierRegistry()) {
    check(SearchComponentConfiguration{search,
                                       {OptionConfiguration{
                                           registration.id,
  ...
  for (const EvaluationTargetRegistration &registration :
    ...
    check(
        SearchComponentConfiguration{
            ...
            OptionConfiguration{registration.id, registration.defaultSettings}},
        "evaluator defaults");
  }
  // The search policy option round-trips.
  {
    ...
    check(SearchComponentConfiguration{OptionConfiguration{search.id, promoted},
  ...
  // custom values round-trips exactly.
  {
    const OptionSettings settings = SettingsWithOverrides(
        kInputInsertionModifierId, {{"minTimeMs", "500"},
                                    {"maxTimeMs", "3000"},
                                    {"seed", "42"},
                                    {"steerMode", "offset"},
                                    {"steerOffsetMin", "-0.3"},
    ...
    check(
        SearchComponentConfiguration{
            search,
            {OptionConfiguration{kInputInsertionModifierId, settings}},
            evaluation},
  ...
  {
    const OptionSettings settings = SettingsWithOverrides(
        kInputInsertionModifierId, {{"minTimeMs", "800"},
                                    {"maxTimeMs", "2600"},
                                    {"seed", "9"},
                                    {"steerMode", "absolute"},
  ...
  {
    const OptionSettings settings = SettingsWithOverrides(
        kExistingEventPerturbationModifierId, {{"minTimeMs", "2000"},
                                               {"maxTimeMs", "5000"},
                                               {"seed", "7"},
                                               {"minCount", "2"},
                                               {"maxCount", "5"},
  ...
  {
    const OptionSettings settings = SettingsWithOverrides(
        kExistingEventPerturbationModifierId, {{"steerMode", "absolute"},
  ...
  {
    const OptionSettings settings = SettingsWithOverrides(
        kInputDeletionModifierId, {{"minTimeMs", "1100"},
                                   {"maxTimeMs", "2200"},
                                   {"seed", "3"},
                                   {"steerMaxCount", "5"},
                                   {"accelerateEnabled", "true"},
    ...
    check(SearchComponentConfiguration{search,
                                       {OptionConfiguration{
                                           kInputDeletionModifierId, settings}},
                                       evaluation},
  ...
  {
    const OptionSettings settings = SettingsWithOverrides(
        kSmoothSteeringModifierId, {{"minTimeMs", "900"},
                                    {"maxTimeMs", "3210"},
                                    {"seed", "11"},
                                    {"deformationCount", "3"},
                                    {"radiusMs", "120"},
    ...
    check(
        SearchComponentConfiguration{
            search,
            {OptionConfiguration{kSmoothSteeringModifierId, settings}},
            evaluation},
        "smooth custom");
  }
  {
    const OptionSettings settings = SettingsWithOverrides(
        kRandomSteeringModifierId,
        {{"minTimeMs", "400"}, {"maxTimeMs", "900"}, {"seed", "1234"}});
    check(
        SearchComponentConfiguration{
            search,
            {OptionConfiguration{kRandomSteeringModifierId, settings}},
            evaluation},
        "random custom");
  }
  ...
  // speed-toward.
  {
    ...
    check(SearchComponentConfiguration{search, DefaultModifierConfigurations(),
                                       OptionConfiguration{
                                           kVelocityEvaluationId, settings}},
          "velocity projected");
  }
  ...
  // switching the measure to projected.
  {
    ...
    if (result.ok) {
      ...
      Check(compiled.at("alignmentEnabled") == "true", "gate preserved");
      Check(compiled.at("minAlignmentPercent") == "25",
            "gate threshold preserved");
      Check(compiled.at("directionZ") == "1", "direction preserved");
    }
  }
  // Explicit evaluation fields (formerly mirrored) round-trip.
  {
    ...
    check(SearchComponentConfiguration{search, DefaultModifierConfigurations(),
                                       OptionConfiguration{
                                           kVolumeEntryEvaluationId, settings}},
          "box entry custom");
  }
  {
    OptionSettings settings =
        FindEvaluationTarget(kCustomVolumeEntryEvaluationId)->defaultSettings;
    settings["plane"] = "xy";
    settings["originX"] = "10";
    settings["originY"] = "-4";
    settings["originZ"] = "2";
    ...
    check(
        SearchComponentConfiguration{
            search, DefaultModifierConfigurations(),
            OptionConfiguration{kCustomVolumeEntryEvaluationId, settings}},
        "prism entry custom");
  }
}
...
void TestCompileStructureErrors() {
  ...
  {
    BlockProgram program = DefaultProgram();
    program.removeBlock(program.find(*program.script())->substack.front());
    const CompileResult result = CompileProgram(program);
  ...
  }
  {
    ...
    const BlockId shift = program.createBlock(
        "mutate/shift-events",
        {{"minCount", "3"}, {"maxCount", "4"}, {"maxTimeShiftMs", "100"}});
  ...
  }
  {
    ...
    const BlockId group = program.createBlock(
        "mutate/window",
        {{"minTimeMs", "1000"}, {"maxTimeMs", "5990"}, {"seed", "55"}});
    const BlockId nudge = program.createBlock("mutate/nudge-steering");
    ...
    if (result.ok) {
      ...
      Check(first.at("minTimeMs") == "1000" && first.at("seed") == "55",
            "first pass shares window");
      Check(second.at("minTimeMs") == "1000" && second.at("seed") == "55",
      ...
      Check(!ValidateSearchComponents(result.configuration,
...
void TestCompileReporterExpressions() {
  ...
  const BlockId sum = program.attachReporter(
      group, "maxTimeMs", "values/add", {{"left", "3000"}, {"right", "2990"}});
  Check(sum != 0, "expression attached");
  const BlockId deformation =
      program.attachReporter(smooth, "deformationCount", "values/maximum",
                             {{"left", "2"}, {"right", "5"}});
  ...
  if (result.ok) {
    const auto found =
        result.configuration.modifiers.front().settings.find("maxTimeMs");
  ...
  const BlockId quotient = program.attachReporter(
      group, "minTimeMs", "values/divide", {{"left", "1000"}, {"right", "0"}});
...
void TestTextRoundTrip() {
  ...
  if (!parsed.program.has_value())
    return;
  const std::string reparsed = PrintBlockProgramText(*parsed.program);
...
void TestTextParseErrors() {
  Check(!ParseBlockProgramText("").program.has_value(), "empty text rejected");
  ...
  Check(!ParseBlockProgramText("search basic-brute-force { bad = }\n")
             .program.has_value(),
...
void TestJsonRoundTrip() {
  ...
  const std::string json = PrintBlockProgramJson(program, remembered);
...
void TestNumberFormatting() {
  ...
  Check(ParseNumberValue("0.5").has_value(), "decimal accepted");
  Check(ParseNumberValue("+.5").has_value(), "leading plus decimal accepted");
...
void TestValidationParity() {
  ...
  Check(!ValidateSearchComponents(compiled.configuration, kSearchTickDurationMs,
                                  6000u)
             .has_value(),
  ...
  const auto error = ValidateSearchComponents(compiled.configuration,
                                              kSearchTickDurationMs, 6000u);
