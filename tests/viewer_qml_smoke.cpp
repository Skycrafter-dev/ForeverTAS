...
int main(int argc, char **argv) {
    ...
    auto *const initialSettingsPanel =
            qobject_cast<QQuickItem *>(
                    root->findChild<QObject *>(
                            QStringLiteral("settingsPanel")));
    auto *const initialSettingsScroll =
            qobject_cast<QQuickItem *>(
                    root->findChild<QObject *>(
                            QStringLiteral("settingsScroll")));
    auto *const initialBlockWorkspace =
            qobject_cast<QQuickItem *>(
                    root->findChild<QObject *>(
                            QStringLiteral("blockWorkspace")));
    auto *const initialExpandBlocksButton =
            qobject_cast<QQuickItem *>(
                    root->findChild<QObject *>(
                            QStringLiteral("expandBlocksButton")));
    ...
    bool globalSettingsVisibleAcrossTabs =
            ...
            initialSettingsPanel != nullptr &&
            initialSettingsScroll != nullptr &&
            initialBlockWorkspace != nullptr &&
    ...
    if (globalSettingsVisibleAcrossTabs) {
        ...
        QCoreApplication::processEvents();
        globalSettingsVisibleAcrossTabs &=
                initialSettingsPanel->property("panelPage").toInt() == 2 &&
                !initialGlobalScript->isVisible() &&
                !initialReplaySection->isVisible() &&
                !initialAppearanceControls->isVisible() &&
                initialSettingsScroll->isVisible() &&
            ...
            [&]() {
                ...
                QTimer::singleShot(500, &application, [&]() {
                    ...
                    const bool blocklyWorkbenchValid =
                            ...
                            !blockEditorBridge.catalogJson().isEmpty() &&
                            blockEditorBridge.catalogJson().contains(
                                    QStringLiteral("\"blocks\"")) &&
                            !blockEditorBridge.workspaceJson().isEmpty() &&
                            blockEditorBridge.workspaceJson().contains(
                                    QStringLiteral("\"blocks\"")) &&
                            blockEditorBridge.darkMode() == controller.darkMode() &&
                            blockEditorBridge.editable();
